...
while True:
    Class_Num = input("Enter The Class Number (11 or 12) or Type 'Exit' To Quit: ")
    
    if Class_Num.upper() == 'EXIT':
        break
    
    if Class_Num not in ['11', '12']:
        print("Invalid Class Number. Please Enter 11 or 12.")
        continue
    ...
    Stream = input("Enter The Stream (Bio, Arts, Commerce, Maths): ").capitalize()
    
    if Stream not in ['Bio', 'Arts', 'Commerce', 'Maths']:
        print("Invalid Stream. Please Enter Bio, Arts, Commerce, or Maths.")
        continue

    Class_Name = f"Class_{Class_Num}_{Section}_{Stream}"

    Class_Teacher_Id = input(f"Enter the Class Teacher ID for {Class_Name}: ")
